  ...
  <div class="user-area">
    ...
    <div class="user-info">
      ...
      <form method="POST" action="{{ route('logout') }}" class="logout-form">
        @csrf
        <button type="submit" class="logout-btn" title="Sair" aria-label="Sair">
...
<main class="main-content">
  <div class="page-header">
    <h1 class="page-title">Editar Nota Fiscal</h1>
  </div>

  <div class="card">
    <form method="POST" action="{{ route('notas.update', $nota->id) }}" id="notaForm">
      @csrf
      @method('PUT')
      ...
      <div class="form-grid">
        <div class="form-group">
          ...
          <input type="text" id="numero" name="numero" class="form-control"
                 placeholder="Ex: NF-2025-0001" value="{{ old('numero', $nota->numero) }}">
        </div>

        <div class="form-group">
          <label for="data">Data</label>
          <input type="date" id="data" name="data" class="form-control"
                 value="{{ old('data', $nota->data ? $nota->data->format('Y-m-d') : '') }}">
        </div>

        <div class="form-group">
          <label for="tipo">Tipo</label>
          <select id="tipo" name="tipo" class="form-control">
            <option value="entrada" {{ old('tipo', $nota->tipo) == 'entrada' ? 'selected' : '' }}>Entrada</option>
            <option value="saida" {{ old('tipo', $nota->tipo) == 'saida' ? 'selected' : '' }}>Saída</option>
          </select>
        </div>
        ...
        <div class="form-group">
          ...
          <input type="text" id="cliente" name="cliente" class="form-control"
                 placeholder="Digite o nome" value="{{ old('cliente', $nota->cliente) }}">
        </div>

        <div class="form-group">
          <label for="valor">Valor Total</label>
          <input type="hidden" id="valor_original" name="valor_original" value="{{ $nota->valor }}">
          <input type="text" id="valor" name="valor" class="form-control"
                 placeholder="R$ 0,00" value="{{ old('valor', number_format($nota->valor, 2, ',', '.')) }}">
        </div>

        <div class="form-group">
          <label for="status">Status</label>
          <select id="status" name="status" class="form-control">
            <option value="processada" {{ old('status', $nota->status) == 'processada' ? 'selected' : '' }}>Processada</option>
            <option value="pendente" {{ old('status', $nota->status) == 'pendente' ? 'selected' : '' }}>Pendente</option>
            <option value="cancelada" {{ old('status', $nota->status) == 'cancelada' ? 'selected' : '' }}>Cancelada</option>
          </select>
        </div>
      </div>
...
<script>
  ...
  // Manipular o envio do formulário para formatar corretamente o valor
  document.getElementById('notaForm').addEventListener('submit', function(e) {
    // Remover formatação do valor antes de enviar
    const valorFormatado = maskValor.unmaskedValue;
    const valorNumerico = parseFloat(valorFormatado);
    
    if (!isNaN(valorNumerico)) {
      // Criar um input hidden para enviar o valor numérico
